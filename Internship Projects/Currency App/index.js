...
async function getExchangeRate() {
    ...
    try{
        const response = await fetch(`https://v6.exchangerate-api.com/v6/28eb26c33360cf597ef2e1cd/latest/${fromCur.value}`);
        const result = await response.json();
        const exchangeRate = result.conversion_rates[toCur.value];
        const totalExRate = (amountVal * exchangeRate).toFixed(3);
